...
async def _run_proposal_evaluation(problem_id: UUID, schema_name: str) -> dict:
    ...
    async with AsyncSession(engine, expire_on_commit=False) as session:
        ...
        # 5. Tell the guide the advisory is ready. ADVISORY ONLY — the status is
        # still PENDING_REVIEW and their decision gate is untouched; this just
        # means they no longer have to poll for the AI to finish. The student is
        # deliberately not told: they hear from their guide, not from the AI.
        try:
            from app.core.notifications.dispatch import notify_user
            from app.core.notifications.models import NotificationType
            await notify_user(
                session,
                notification_type=NotificationType.RESEARCH_PROPOSAL_EVALUATED,
                recipient_user_id=problem.guide_user_id,
                title=f"AI screening complete: {problem.title}",
                body=(f"AI recommends {recommendation} (novelty {novelty_score:.0%}, "
                      f"feasibility {feasibility_score:.1f}/10, clarity {clarity_score:.1f}/10). "
                      "Advisory only — your decision is required."),
                entity_type="ResearchProblem",
                entity_id=str(problem_id),
            )
            await session.commit()
        except Exception:
            logger.exception("proposal eval: guide notification skipped for %s", problem_id)
