...
from app.core.calendar.models import AcademicEvent
...
class CalendarService:
    ...
    async def get_student_calendar(
        ...
        # 1. Admin/dean-declared events & holidays, visible to this student
        event_sql = text("""
            SELECT id, title, event_type, start_at, end_at, is_all_day, visibility
        ...
        assignment_sql = text("""
            SELECT a.id, a.title, a.due_date, a.syllabus_id
            FROM assignments a
        ...
        for r in rows:
            ...
            items.append(CalendarItem(
                id=str(r["id"]), title=f"Assignment due: {r['title']}",
        ...
        lab_sql = text("""
            SELECT la.id, la.title, la.deadline
        ...
        for r in rows:
            ...
            items.append(CalendarItem(
                id=str(r["id"]), title=f"Lab due: {r['title']}",
        ...
        # 4. Exam schedule (m11_sis)
        exam_sql = text("""
            SELECT sch.course_id, c.title AS course_title, sch.exam_date, sch.start_time, sch.end_time
        ...
        for r in rows:
            items.append(CalendarItem(
                id=f"exam-{r['course_id']}-{r['exam_date']}", title=f"Exam: {r['course_title']}",
