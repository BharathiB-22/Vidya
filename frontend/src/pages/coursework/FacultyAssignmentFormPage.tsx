...
import type { CourseworkType } from '@/types/coursework'
...
export default function FacultyAssignmentFormPage() {
  ...
  const { data: existing } = useAssignment(id ?? '')
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (isEdit) {
        ...
        const created = await create.mutateAsync({ ...payload, due_date: payload.due_date!, syllabus_id: syllabusIdFromQuery })
        addToast('Assignment created as draft.', 'success')
        navigate(`/faculty/assignments/${created.id}/submissions`)
        return
      }
