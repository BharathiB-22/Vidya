    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-1.5 flex flex-wrap gap-2">
          {student.program && <span className="text-xs text-slate-400">{student.program.name} · {student.program.degree_type}</span>}
          {student.batch && <span className="text-xs text-slate-500">Batch {student.batch.start_year}–{student.batch.end_year}</span>}
          {student.current_section && <span className="text-xs text-slate-500">Sec {student.current_section.name}</span>}
          {student.admission_year && <span className="text-xs text-slate-500">Admitted {student.admission_year}</span>}
        </div>
      </div>
    </div>
  )
  ...
  return (
    <PageShell>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <select value={programId} onChange={e => { setProgramId(e.target.value); setBatchId(''); setPage(1) }}
