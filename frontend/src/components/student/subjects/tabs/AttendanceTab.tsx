...
export function AttendanceTab({ subject }: SubjectTabProps) {
  ...
  if (isLoading) return <div className="text-sm text-gray-600 py-8 text-center">Loading attendance…</div>
  if (isError || !data) {
    return (
      <div className="text-center py-12 rounded-xl border border-dashed border-gray-200">
        ...
        <p className="text-sm text-gray-600">No attendance records for this subject yet.</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded-xl border border-gray-200 divide-y divide-gray-100 bg-white overflow-hidden max-h-96 overflow-y-auto">
        {sessions.length === 0 ? (
          <div className="px-5 py-8 text-center text-sm text-gray-600">No sessions recorded yet.</div>
        ) : (
          sessions.map((s) => (
            <div key={s.session_id} className="px-5 py-2.5 flex items-center justify-between text-sm">
              <div>
                ...
                {s.topic_covered && <span className="text-gray-600 ml-2 text-xs">{s.topic_covered}</span>}
              </div>
